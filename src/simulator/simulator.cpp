...
{                                             // Used for Leds, probes, for update at frame rate 50 fps
    ...
    {                                       // Used for reactive elements
        ...
        {                                   // Used for all non-reactive elements
            ...
            for( it=m_runStepList.begin(); it!=m_runStepList.end(); it++ )
                (*it)->runStep();
        }
...
void Simulator::runContinuous()
{
    ...
    //emit runMcu(); // call avr to start its timer
...
void Simulator::addToRunList( eElement* el )
{
    m_runStepList.push_back( el );
}
void Simulator::remFromRunList( eElement* el )
{
    std::vector<eElement*>::iterator it;

    for( it=m_runStepList.begin(); it!=m_runStepList.end(); it++ )
        if( *it == el ) { m_runStepList.erase(it); break; }
}
